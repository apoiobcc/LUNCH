...
class TestSchedule:
    ...
    def test_available(self):
        ...
        assert ans['available'] == correct['available']
    
    def test_available_with_restriction(self):
        wparser = ParserWorkload("test_input_file/test_workload.csv")
        wparser.parse()
        self.p.setRestriction(wparser.service)        
        ans = self.p.parse()
        self.p.setRestriction(dict())
        correct = {
            'available':[
                ["pmiranda",121,0],
                ["pmiranda",122,0],
    ...
    def test_noAnswer_with_restrictions(self):
        wparser = ParserWorkload("test_input_file/test_workload.csv")
        wparser.parse()
        self.p.setRestriction(wparser.service)
        no = self.p.noAnswer("test_input_file/test_workload.csv", 7)
