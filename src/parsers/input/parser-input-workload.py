...
def assembleWorkload(info, semester):
    ...
    workload = ""
    for atom in info:
        if atom["semester"] != semester:
            continue
        workload = (
            workload
            + f"course({atom['course']},{atom['group']},{atom['teacher']})."
            + "\n"
        )
    return workload[:-1]
...
def main():
    ...
    with open(
        f"clingo_input_files/course1s{file_name[-6:-4]}.txt", "a"
    ) as clingo_input_file:
        clingo_input_file.write(s1)
    with open(
        f"clingo_input_files/course2s{file_name[-6:-4]}.txt", "a"
    ) as clingo_input_file:
