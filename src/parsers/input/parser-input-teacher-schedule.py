...
def main():
    ...
    preferable = ""
    for i in info:
        available = available + assembleDayPredicate(
            "available", i["teacher"], i["available"]
        )
        preferable = preferable + assembleDayPredicate(
            "preference", i["teacher"], i["preference"]
        )
    with open(
        f"clingo_input_files/available{file_name[-8:-4]}.txt", "a"
    ) as clingo_input_file:
        clingo_input_file.write(available[:-1])
    with open(
        f"clingo_input_files/preferable{file_name[-8:-4]}.txt", "a"
    ) as clingo_input_file:
