...
class TestClass(unittest.TestCase):
    ...
    out3 = [
        ["class(A,11:00)", "class(B,12:00)", "class(C,13:00)"],
        ["class(D,11:00)", "class(E,12:00)"],
        ["class(F,11:00)", "class(G,13:00)"],
    ]
    exp3 = [
        ["class(G,13:00)", "class(F,11:00)"],
        ["class(B,12:00)", "class(A,11:00)", "class(C,13:00)"],
        ["class(E,12:00)", "class(D,11:00)"],
    ]

    exp4 = [
        ["class(G,13:00)", "class(F,11:00)"],
        ["class(B,12:00)", "class(ZZZZZZZ,11:00)", "class(C,13:00)"],
        ["class(E,12:00)", "class(D,11:00)"],
    ]
    ...
    def test_true_trivial_answers(self):
        self.assertTrue(
            compare_answers(self.out0, self.exp0), "Error in the trivial test"
        )

    def test_true_different_predicates_order(self):
        self.assertTrue(
            compare_answers(self.out1, self.exp1), "Erro when changing predicates order"
        )

    def test_true_different_answers_order(self):
        self.assertTrue(
            compare_answers(self.out2, self.exp2), "Error when changing answer order"
        )

    def test_true_different_answers_predicates_order(self):
        self.assertTrue(
            compare_answers(self.out3, self.exp3),
            "Error when changing answers and predicates order",
        )

    def test_false_different_predicate(self):
        self.assertFalse(
            compare_answers(self.out3, self.exp4),
            "Error when the predicate is different",
        )

    def test_false_extra_answer(self):
        self.assertFalse(
            compare_answers(self.out3, self.exp5),
            "Error when the size of the answers sets doesn't match",
        )

    def test_false_missing_predicate(self):
        self.assertFalse(
            compare_answers(self.out3, self.exp6),
            "Error when the size of the predicates set doesn't match",
        )

    def test_false_missing_answer(self):
        self.assertFalse(
            compare_answers(self.out3, self.exp7),
            "Error when there is a missing answer",
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            "Usage: python3 sat-answers-comparator.py {expected answers file} < {clingo output file}"
        )
